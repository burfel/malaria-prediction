...
In our case a parametric model was appropriate as we could identify linear relationships between variables and the response variable and the distribution of the residuals in our linear model (with parasitemia percentage as independent variable) showed fairly normal distributions (see chapter _Methods_ [INSERT LINK HERE]). Also, there was not enough data available to use more elaborate Machine Learning (ML) methods, including Random Forest (RF), Support Vector Machines (SVMs), Ridge, Lasso or Elastic net; it would not have been meaningful to divide the data into two separate data sets (a training and a test data set).
...
Another aspect we considered when designing the models was which type of data a user is most likely to have. 
