...
They can get the direct result from the output computed based on a logistic regression model and optionally compare it to the linear regression model shown in the plot. The tool allows zooming and saving the plot as well as showing the exact coordinates when hovering over the plot. Optionally, the user can perform different statistical tests to compare the available models, including the likelihood ratio test. 
