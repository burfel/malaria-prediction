...
The plot of the relationship between percentage of parasetemia and the outcome variable is shown in figure X [INSERT NUMBER] in subchapter _Model selection_.

* Multivariate normality–multiple regression assumes that the **residuals are normally distributed**. The **Q-Q plot** in figure X [INSERT NUMBER] shows that the residuals of the linear model with parasitemia percentage roughly follow a normal distribution. The two outliers we identified (sample 35 and 39) could be discarded but due to the very few data available, we decided not to but accepted it to represent variance in the biological data set. We can see these two outliers as well as other possible intuitive outliers (samples 7, 10, 11, 19) in figure X [INSERT NUMBER HERE].
...
[INSERT AUTP CORRELATION PLOT HERE]
...
Checking the plots in figure X [INSERT NUMBER HERE] and the statistics [INSERT STATISTICS HERE], we can see that for a linear model with parasitemia percentage the model assumptions roughly hold, and with using the original outcome variable we can fit a line that is a good representation of the relationship. However, for a more complex model (eg including white blood cells) and for the analysed mouse data set the assumptions clearly did not hold (see figure X). The residuals were not uniformally distributed (the response variable does not have to be Gaussian!).
